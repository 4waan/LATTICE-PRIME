...
contract AxeBoardTest is Test, PolicyFixture {
    ...
    function test_zeroOutstandingOrZeroWindowCannotBeDeployed() public {
        ...
        new AxeBoard(
            ISealedOrderBook(address(book)), registry, params, PROBE_FEE, AXE_BOND, ANSWER, OSR, 0
        );
    ...
    function test_wrongBondIsRefused() public {
        ...
        board.postAxe{value: 1}(
            grid, AXE_SALT, uint64(block.timestamp + 1), AxeBoard.Respondent.ALL, bytes32(0), false
        );
    }
    ...
    function test_anAlreadyExpiredAxeIsRefused() public {
        ...
        board.postAxe{value: AXE_BOND}(
            grid, AXE_SALT, uint64(block.timestamp), AxeBoard.Respondent.ALL, bytes32(0), false
        );
    }
    ...
    function test_specifiedNeedsASetAndTheOthersRefuseOne() public {
        ...
        board.postAxe{value: AXE_BOND}(
            grid, AXE_SALT, uint64(block.timestamp + 1), AxeBoard.Respondent.SPECIFIED, bytes32(0), false
        );
    ...
    function test_postingTwiceIsRefused() public {
        ...
        vm.expectRevert();
        board.postAxe{value: AXE_BOND}(
            grid, AXE_SALT, uint64(block.timestamp + 7 days), AxeBoard.Respondent.ALL, bytes32(0), false
        );
    }
    ...
    function test_outstandingIsCapped() public {
        ...
        address[4] memory who =
            [BORROWER, MALLORY, MAKER, address(0xD00)];
        ...
        vm.prank(extra);
        vm.expectRevert(abi.encodeWithSelector(AxeBoard.TooManyOutstanding.selector, MAX_OUT));
        board.probe{value: PROBE_FEE}(axeId, cellYes, bytes32(0), NONE);
    }
    ...
    function test_silenceOnAnOpenProbePaysTheProberOnce() public {
        ...
        board.slash(a);
        assertEq(board.credit(BORROWER), AXE_BOND + PROBE_FEE, "injured party, not the caller");
    ...
    function test_reclaimNeedsTheAxeToBeOverAndToOweNothing() public {
        bytes32 axeId = _post(false);
        vm.expectRevert(
            abi.encodeWithSelector(AxeBoard.AxeStillGood.selector, uint64(block.timestamp + 7 days))
        );
        ...
        vm.prank(LENDER);
        board.withdrawAxe(axeId);
        vm.expectRevert(abi.encodeWithSelector(AxeBoard.ProbesOutstanding.selector, uint32(1)));
        board.reclaimAxe(axeId);
    ...
    function test_dischargeRequiresTheLendersOwnLaterUncancelledCommitment() public {
        ...
        vm.prank(LENDER);
        vm.expectRevert(
            abi.encodeWithSelector(AxeBoard.ProbeNotIndicated.selector, AxeBoard.Status.OPEN)
        );
        ...
        vm.prank(LENDER);
        vm.expectRevert(abi.encodeWithSelector(AxeBoard.OrderIsNotTheLenders.selector, BORROWER));
        board.discharge(probeId, theirs);

        vm.prank(LENDER);
        vm.expectRevert(
            abi.encodeWithSelector(
                AxeBoard.OrderPredatesTheIndication.selector, uint64(block.timestamp - 1), uint64(block.timestamp)
            )
        );
    ...
    function test_aSpecifiedSetAdmitsOnlyItsMembers() public {
        ...
        bytes32 axeId = board.postAxe{value: AXE_BOND}(
            grid, AXE_SALT, uint64(block.timestamp + 7 days), AxeBoard.Respondent.SPECIFIED, root_, false
        );
    ...
    function test_theBandedSetMetersTheProbeAndRefusesTheReveal() public {
        ...
        assertEq(board.breakingSize(ROW_SIZE, L.G_BUCKET), 3, "two announce, the third is silent");
    ...
    function test_row13BudgetUsesTheRectangularDomain() public {
        ...
        assertEq(r.domainBits, AxeGrid.RECTANGLE_BITS, "the budget is set against the domain");
    ...
    function _post(bool priv) internal returns (bytes32 axeId) {
        ...
        axeId = board.postAxe{value: AXE_BOND}(
            grid, AXE_SALT, uint64(block.timestamp + 7 days), AxeBoard.Respondent.ALL, bytes32(0), priv
        );
    }
    ...
    function _withoutRow(uint16 row) internal pure returns (ParameterRoot.Param[] memory out) {
        ParameterRoot.Param[] memory set = asDeployed();
        out = new ParameterRoot.Param[](set.length - 1);
        uint256 k;
        for (uint256 i = 0; i < set.length; ++i) {
            if (set[i].key == bytes32(uint256(row))) continue;
            out[k++] = set[i];
        }
    }
...
contract AxeHandler is Test {
    ...
    constructor(AxeBoard board_, OrderBook book_, address lender_, bytes32 axeId_, CachedAxe cache)
    {
...
contract AxeBoardInvariantTest is Test, PolicyFixture {
    ...
    function invariant_etherIsConserved() public view {
        assertEq(address(board).balance, handler.ghost(), "balance == undeclared credits and bonds");
    }
