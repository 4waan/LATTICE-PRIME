...
// Private trading is a candidate stack. `client.json` is an on-chain read and
// does not bind session factories or routers. Vercel has no /api/private/status,
// so the generated client still names the DualRegistrationGate epoch. A later
// bound release (enabled, not an overlay) replaces this stub as-is.
const PRIVATE_TRADING_ACTIVATION_EPOCH = 8;

function shippedPrivateTrading(existing) {
    if (existing && existing.enabled === true && existing.overlay !== true) {
        return existing;
    }
    const epoch = Number(existing?.activationEpoch) > 0
        ? Number(existing.activationEpoch)
        : PRIVATE_TRADING_ACTIVATION_EPOCH;
    return {
        ...(existing && typeof existing === "object" ? existing : {}),
        enabled: existing && existing.overlay === true ? !!existing.enabled : false,
        activationEpoch: epoch,
        reason: existing?.reason
            || ("Private trading is not bound on this deployment yet. DualRegistrationGate is pending for epoch "
                + epoch + "."),
    };
}
...
const shippedClient = {
    ...client,
    privateTrading: shippedPrivateTrading(client.privateTrading),
};

const bundle =
    "// Generated by tools/gen-app.mjs. Do not hand edit.\n" +
    "const CLIENT = " + JSON.stringify(shippedClient) + ";\n" +
