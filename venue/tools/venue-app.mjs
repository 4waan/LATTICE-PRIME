    ...
    try {
        ...
        if (Venue.page === "venue") {
            ...
            if (epoch !== Venue._venueEpoch || now - (Venue._venueAt || 0) > 60000) {
                ...
                // Once per session, so the HCS evidence tile is filled in on
                // first paint rather than waiting for the Evidence tab.
                await Venue.refreshIssuerEvidence?.({lazy: true}).catch(() => {});
            }
        }
        Venue.pollMs = 5000;
        clearInterval(Venue.timer);
...
Venue.paintIncome = function () {
    ...
    if (summaryAction) {
        ...
        } else if (state.proof && state.proof.merkleValid == null) {
            Venue._incomeSummaryAction = "refresh";
