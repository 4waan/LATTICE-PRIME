...
//
// It **cannot forge.** Every `charge` and `refusal` names a transaction hash and
// a log index, and `tools/hcs-verify.mjs` fetches that log back and compares it
// field for field. Every `silence` names a transaction that a third party can
// fetch and see succeeded with no charge on the row.
//
...
import {
    SITES, SOURCES, SOURCE_ADDRESS_KEY, TOPIC_CHARGED, TOPIC_REFUSED,
    encode, decode, keyOf, describe, RecordError,
} from "./hcs.mjs";
...
const blank = () => ({
    ...
    sources: Object.fromEntries(SOURCES.map((s) => [s, {throughTs: "0.0", checkpointedThrough: null}])),
    published: 0,
    lastRun: null,
});
...
function loadCursor() {
    ...
    for (const s of SOURCES) {
        if (j.sources && j.sources[s]) base.sources[s] = {...base.sources[s], ...j.sources[s]};
    }
...
if (has("--from")) {
    ...
    for (const s of SOURCES) cursor.sources[s] = {throughTs: from + ".0", checkpointedThrough: null};
...
function decodeRefusal(log) {
    if (log.topics.length !== 2) throw new Error("DisclosureRefused wants one indexed field");
    const data = String(log.data).replace(/^0x/, "");
    if (data.length !== 128) throw new Error(`DisclosureRefused data is ${data.length / 2} bytes, want 64`);
    return {
        id: "0x" + log.topics[1].replace(/^0x/, "").toLowerCase(),
        row: num("0x" + data.slice(0, 64)),
        x: num("0x" + data.slice(64, 128)),
    };
}
...
async function recordsFor(src, fromTs, toTs) {
    const address = c.addresses[SOURCE_ADDRESS_KEY[src]];
    ...
    for (const r of results) {
        ...
        // Refusals first: they belong to the transaction whether or not its
        // selector is one this relay watches, because a refusal is the ceiling
        // speaking rather than the budget.
        for (const l of logs) {
            if (String(l.topics[0]).toLowerCase() !== TOPIC_REFUSED) continue;
            const d = decodeRefusal(l);
            out.push({v: 1, k: "refusal", c: src, tx, li: Number(l.index), id: d.id, r: d.row, x: d.x});
        }
        ...
        for (const l of logs) {
            ...
            out.push({
                v: 1, k: "charge", c: src, tx, li: Number(l.index),
        ...
        for (const row of site.rows) {
            ...
            out.push({
                v: 1, k: "silence", c: src, tx, sel, fn: site.fn,
...
async function checkpointsFor(src, epochs, blk) {
    ...
    for (const e of [...new Set(epochs)].sort((a, b) => a - b)) {
        ...
        out.push({v: 1, k: "checkpoint", c: src, e, blk, rows});
    }
    return out;
}
